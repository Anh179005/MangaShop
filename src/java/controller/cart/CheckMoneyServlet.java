 ...
 * @author ACER
 */
@WebServlet(name="CheckMoneyServlet", urlPatterns={"/checkmoney"})
public class CheckMoneyServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CheckMoneyServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CheckMoneyServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        if (!loggedIn) {
            ...
            if(tongtien > acc.getBudget()){
                request.setAttribute("error", "Bạn còn thiếu " + (tongtien-acc.getBudget()) + " trong tài khoản, hãy nạp thêm");
                request.getRequestDispatcher("deposit.jsp").forward(request, response);
            } else {
                session.setAttribute("tongtien",tongtien);
                session.setAttribute("budget",acc.getBudget());
                String dest= "checkout?tongtien="+tongtien+"&budget="+acc.getBudget();
